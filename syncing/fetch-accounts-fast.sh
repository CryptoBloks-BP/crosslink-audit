...
# Fast version of fetch-accounts.sh with smaller batch sizes
...
if [ "$CONTRACT" != "x.libre" ] && [ "$CONTRACT" != "v.libre" ]; then
    ...
    echo "Usage: $0 [network] [contract] [batch_size]"
    echo "  network: mainnet (default) or testnet"
    echo "  contract: x.libre (default) or v.libre"
    echo "  batch_size: number of accounts per batch (default: 50)"
    exit 1
fi
...
echo "Fetching accounts from $CONTRACT on $NETWORK (batch size: $BATCH_SIZE)..."
...
OUTPUT_FILE="${CONTRACT}_${NETWORK}_accounts_with_balances_fast.csv"
...
process_accounts() {
    ...
    echo "$response" | jq -r '.rows[] | [.account, .btc_address // "none"] | @csv' | while IFS=',' read -r account btc_address; do
        ...
        # Get BTC balance
        echo "Processing account: $account (BTC address: $btc_address) - Batch $batch_count, Total: $total_processed" >&2
